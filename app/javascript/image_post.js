        ...
        return function(e){
          ...
          if (imageLength < 3) {
            $("#image-input").append(`<label for="item_images${labelLength+1}" class="sell-container__content__upload__items__box__label" data-label-id="${labelLength+1}">
                                        <input multiple="multiple" class="dropzone" id="item_images${labelLength+1}" type="file" name="crop[images][]">
...
$(document).on('click', '.preview-image__button__delete', function(){
  ...
  if (imageLength ==3) {
    ...
    $("#image-input").append(`<label for="item_images${labelLength+1}" class="sell-container__content__upload__items__box__label" data-label-id="${labelLength+1}">
                                <input multiple="multiple" class="dropzone" id="item_images${labelLength+1}" type="file" name="item[images][]">
                              </label>`);
  };
});
